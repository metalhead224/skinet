...
export class CheckoutPaymentComponent implements OnInit {
  ...
  ngOnInit(): void {
    loadStripe(
      'pk_test_51NeBSOII1js6OW9WBrYNJkEc2e5NJs4i89nngy2eXUiKTl5VYV35PnVjVlZCsDYnzANaOesEI3BJgl5bJ7mpv9GS00OPe4TOQE'
    ).then((stripe) => {
      this.stripe = stripe;
  ...
  async submitOrder() {
    ...
    try {
      ...
      const paymentResult = this.confirmPaymentWithStripe(basket);
      if ((await paymentResult).paymentIntent) {
        this.basketService.deleteBasket(basket);
        const navigationExtras: NavigationExtras = { state: createdOrder };
        this.router.navigate(['checkout/success'], navigationExtras);
      } else {
        this.toastr.error((await paymentResult).error?.message);
      }
    } catch (error: any) {
      console.log(error);
